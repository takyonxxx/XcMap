...
//private slot
void CompositeTileSourceConfigurationWidget::addTerrainLayer()
{
    QSharedPointer<CompositeTileSource> composite = _composite.toStrongRef();
    if (composite.isNull())
        return;

    QSharedPointer<OSMTileSource> source(new OSMTileSource(OSMTileSource::TerrainTiles));
    composite->addSourceTop(source);
}

//private slot
void CompositeTileSourceConfigurationWidget::addSatLayer()
{
    QSharedPointer<CompositeTileSource> composite = _composite.toStrongRef();
    if (composite.isNull())
        return;

    QSharedPointer<OSMTileSource> source(new OSMTileSource(OSMTileSource::SatTiles));
    composite->addSourceTop(source);
}
...
void CompositeTileSourceConfigurationWidget::init()
{
    ...
    menu->addAction("MapQuest-OSM Tiles", this, SLOT(addTerrainLayer()));
    menu->addAction("MapQuest Open Aerial Tiles", this, SLOT(addSatLayer()));
