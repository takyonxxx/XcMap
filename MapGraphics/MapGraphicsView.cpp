...
void MapGraphicsView::rotate(const qreal &angle)
{
    if (_tileSource.isNull())
        return;
...
void MapGraphicsView::setSpacing(quint8 spacing)
{
   _spacing = spacing;
}
...
void MapGraphicsView::setZoomLevel(quint8 nZoom, ZoomMode zMode)
{
    ...
    mousePoint = QPointF(sceneRect.width()*xRatio - _spacing,sceneRect.height()*yRatio  - _spacing) - offset;
...
void MapGraphicsView::doTileLayout()
{
    ...
    const QPointF centerPointQGS = _childView->mapToScene(_childView->width()/2.0,_childView->height()/2.0);
